...
void LCDPutStr2(const char *str , char pos , char ln)
{
    uint8_t x = pos;
    uint8_t y = ln;
    if ((y > (NB_LINES-1)) || (x > (NB_COL-1)))
    {
        return;
    }
    LCDPutCmd((y == 1) ? (0xC0 | x) : (0x80 | x));
    __delay_ms(LCD_delay);
...
void LCDPutStr3(const char *str , char ln)
{
    uint8_t x = 8 -((strlen(str))/2);
    uint8_t y = ln;
    if ((y > (NB_LINES-1)))
    {
        return;
...
void LCD_Clear(){
    LCDPutCmd(0);  // Clear the LCD
    LCDPutCmd(1);  // Move the cursor to first position
}
